...
#define CPM_ANA_CLK_ENABLE_NOIRQ() \
    do { \
        uint32_t __ana_if_ahb_cfg = XH_PSO->ANA_IF_AHB_CFG; \
        uint32_t __ana_if_clk_cfg = XH_PSO->ANA_IF_CLK_CFG; \
...
{
    // Close ANA if clock
    if (ana_if_ahb_cfg != CPM_ANA_CLK_IGNORE) XH_PSO->ANA_IF_AHB_CFG = ana_if_ahb_cfg;
    if (ana_if_clk_cfg != CPM_ANA_CLK_IGNORE) XH_PSO->ANA_IF_CLK_CFG = ana_if_clk_cfg;
    // Wait
...
{
    // Close ANA if clock
    register_set0(&XH_PSO->ANA_IF_AHB_CFG, CPM_ANA_IF_AHB_GATE_EN_MASK);
    XH_PSO_UPD_RDY();
}
...
{
    // Close ANA if clock
    register_set1(&XH_PSO->ANA_IF_AHB_CFG, CPM_ANA_IF_AHB_GATE_EN_MASK);
    XH_PSO_UPD_RDY();
}
