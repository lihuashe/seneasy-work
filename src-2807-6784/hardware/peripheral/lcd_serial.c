 ...
 * @addtogroup 
 * @ingroup 
 * @details LCD serial interface driver, Used for XH98xx/XH98xx/XH98xx
 *
 * @{
 */
 ...
 **/
static void lcd_sw_config(XH_SF_Type *sf, const lcd_config_t *config, uint32_t byteCnt)
{
    // Write sw cfg0 REG
    REGW(&sf->SW_SPI_CFG0_REG,MASK_4REG(SF_SW_CFG0_CMD_P0_BIT_CNT,        8,
                                        SF_SW_CFG0_CMD_P0_BUS_WIDTH,      0,
                                        SF_SW_CFG0_CMD_P1_BUS_WIDTH,      0,
                                        SF_SW_CFG0_CMD_P1_BIT_CNT,        24));

    // Write sw cfg1 REG
    REGW(&sf->SW_SPI_CFG1_REG,MASK_4REG(SF_SW_CFG1_SDATA_BYTE_CNT,        byteCnt,
                                        SF_SW_CFG1_SDATA_BUS_WIDTH,       config->line_mode,
                                        SF_SW_CFG1_BUF_WIDTH_BYTES,       config->rgb_mode==SFB_LCD_RGB565 ? BUF_WIDTH_BYTES_RGB565 : BUF_WIDTH_BYTES_RGB666_RGB888,
                                        SF_SW_CFG1_SW_CFG_EN,             SW_CFG_ENABLE));
}
 ...
 * @param[in] sf  sf object
 **/
static void lcd_sw_enable(XH_SF_Type *sf, uint8_t isEnable)
{
    /* Enable or Disable software cfg */
    REGW(&sf->SW_SPI_CFG1_REG,MASK_1REG(SF_SW_CFG1_SW_CFG_EN, isEnable));
 ...
 * @param[in] config  config
 **/
static void lcd_config_ex(XH_SF_Type *sf, uint32_t cs, const lcd_config_t *config)
{
 ...
 * @return 
 **/
void lcd_config(XH_SF_Type *lcd, uint32_t cs, const sfb_config_t *sfconfig, const lcd_config_t *lcdconfig)
{
    sfb_config(lcd, cs, sfconfig);
    lcd_config_ex(lcd, cs, lcdconfig);
 ...
 * @return 
 **/
void lcd_write_reg(XH_SF_Type *lcd, uint32_t cs, uint8_t reg, const void *data, uint32_t data_bytes)
{
 ...
 * @return 
 **/
void lcd_read_reg(XH_SF_Type *lcd, uint32_t cs, uint8_t reg, uint8_t dummy_bits, const void *data, uint32_t data_bytes)
{
    sfb_rw_params_t param;
    lcd_config_t config;
    uint32_t sfb_reg;
 ...
 * @return 
 **/
void lcd_write_mem(XH_SF_Type *lcd, uint32_t cs, uint8_t reg, const void *data, uint32_t data_bytes)
{
    sfb_rw_params_t param;
 ...
 * @return 
 **/
void lcd_write_mem_ex(XH_SF_Type *lcd, uint32_t cs, sfb_keep_cs_t keep_cs, uint8_t reg, const void *data, uint32_t data_bytes)
{
    sfb_rw_params_t param;
 ...
 * @return 
 **/
void lcd_write_reg_4lines(XH_SF_Type *lcd, uint32_t cs, uint8_t reg0, uint32_t reg1,uint8_t cmdBits)
{
    sfb_rw_params_t param;
    lcd_config_t config;
    uint32_t sfb_reg;
 ...
 * @return 
 **/
void lcd_write_reg_and_data_4lines(XH_SF_Type *lcd, uint32_t cs, uint8_t reg, const void *data, uint32_t data_bytes)
{
    sfb_rw_params_t param;
    lcd_config_t config;
    uint32_t sfb_reg;
 ...
 * @return 
 **/
void lcd_write_mem_ex_4lines(XH_SF_Type *lcd, uint32_t cs, sfb_keep_cs_t keep_cs, uint8_t reg, const void *data, uint32_t data_bytes)
{
    sfb_rw_params_t param;
    lcd_config_t config;
    ...
    lcd_sw_config(lcd, &config, data_bytes);
