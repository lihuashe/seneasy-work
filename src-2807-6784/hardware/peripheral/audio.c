...
{
    //log_debug("audio_open_timer_handler %d\n", audio_env.state);
...
//   REGW(&XH_AUDIO->CODEC_ANA_CTRL_2, MASK_1REG(AU_SEL_INNERIREF_AUVREF_VBAT_AU, 0));
    ...
    REGW(&XH_AUDIO->CODEC_ANA_CTRL_2, MASK_2REG(AU_CTRL_AU_AULDO28, 2, AU_CTRL_AU_AULDO12,3));
    REGW(&XH_AUDIO->CODEC_ANA_CTRL_2, MASK_1REG(AU_AU_LDO28_MODE, 0));//0:1.8V, 1:2.5V
    /// t1 open ldo_28
    ...
    //open pga intput enable
    REGW(&XH_AUDIO->CODEC_ANA_CTRL_1, MASK_1REG(AU_EN_PGAIN_OPEN, 0));
    
    //Set 1 to select the out-chip 2.8V ldo��?Set 1 to select the in-chip 2.8V ldo
    REGW(&XH_AUDIO->CODEC_ANA_CTRL_2, MASK_1REG(AU_EN_LDO28_OFFCHIPCAP, 1));    
    
    pmu_ana_enable(true, PMU_ANA_ADC);   
    /* must set the following 2 lines in order to make audio adc work */
