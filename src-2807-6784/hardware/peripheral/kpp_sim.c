 ...
 * @return 
 **/
static void kpp_sim_depressed(const kpp_sim_data_t data)
{
    if(memcmp(kpp_sim_env.data, data, sizeof(kpp_sim_data_t)))
    {
        kpp_sim_env.is_report_pressed = true;

        memcpy(kpp_sim_env.data, data, sizeof(kpp_sim_data_t));
...
static void kpp_sim_released(void)
{
    if(kpp_sim_env.is_report_pressed)
    {
        ...
        memset(kpp_sim_env.data, 0, sizeof(kpp_sim_data_t));
    }
}
