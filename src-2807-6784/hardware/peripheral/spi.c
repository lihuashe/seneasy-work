 ...
 * @details 
 *
 * XH98xx does not have SPI module.
 *
 * @{
 */
 ...
 * @return 
 **/
static XH_DMA_CH_Type *spi_tx_dma_config(XH_SPI_Type *spi, XH_DMA_CH_Type *dma, dma_callback_t callback)
{
    if(dma == NULL)
        dma = dma_allocate();
    ...
    if(dma)
    {
        ...
        dconfig.slave_id       = SPI_MST0_TX_DMA_ID;
 ...
 * @return 
 **/
static XH_DMA_CH_Type *spi_rx_dma_config(XH_SPI_Type *spi, XH_DMA_CH_Type *dma, dma_callback_t callback)
{
    if(dma == NULL)
        dma = dma_allocate();
    ...
    if(dma)
    {
        ...
        dconfig.slave_id       = SPI_MST0_RX_DMA_ID;
 ...
 * @return None
 **/
void spi_open(XH_SPI_Type *spi, spi_mode_t mode, spi_transmode_t transmode, uint32_t speed)
{
    int div = 0;
    ...
    if(spi == XH_SPI0)
    {
        register_set1(&XH_PSO->SPI0_CFG, CPM_SPI0_SOFT_RESET_MASK);
        register_set0(&XH_PSO->SPI0_CFG, CPM_SPI0_GATE_EN_MASK);
        XH_PSO_UPD_RDY();
    }
#if 0
    else
    {
        register_set1(&XH_PSO->SPI1_CFG, CPM_SPI1_SOFT_RESET_MASK);
        register_set0(&XH_PSO->SPI1_CFG, CPM_SPI1_GATE_EN_MASK);
        XH_PSO_UPD_RDY();
    }
#endif
 ...
 * @return 
 **/
void spi_close(XH_SPI_Type *spi)
{
    if(spi == XH_SPI0)
        register_set1(&XH_PSO->SPI0_CFG, CPM_SPI0_GATE_EN_MASK);
#if 0
    else
        register_set1(&XH_PSO->SPI1_CFG, CPM_SPI1_GATE_EN_MASK);
#endif
 ...
 * @return 
 **/
void spi_dma_config(XH_SPI_Type *spi, spi_dma_t *dma, dma_callback_t callback)
{
    spi->DMACR = 0;
 ...
 * @return None
 **/
void spi_master_cs_low(XH_SPI_Type *spi)
{
 ...
 * @return None
 **/
void spi_master_cs_high(XH_SPI_Type *spi)
{
 ...
 * @return None
 **/
void spi_master_exchange(XH_SPI_Type *spi, const uint8_t *txbuf, uint8_t *rxbuf, uint32_t len)
{
    ...
    while(rx_cnt < len)
    {
        ...
        if(tx_cnt < len)
        {
            if(register_get(&spi_stat, MASK_POS(SPI_STAT_TX_BYTE_CNT)) < (SPI_TX_FIFO_NUM-1))
            {
 ...
 * @return 
 **/
void spi_master_exchange_dma(XH_SPI_Type *spi, spi_dma_t *dma, const uint8_t *txbuf, uint8_t *rxbuf, uint32_t len)
{
    if(len == 0)
        return;
