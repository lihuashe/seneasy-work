...
/**
 * XH98xx DMA req/ack allocation (according to LaoXing):
 * 
 * F: w_i2c2_dma_rx_req
 * E: w_i2c2_dma_tx_req
 * D: w_i2s_dma_rx_req
 ...
 * 9: w_timer2_dma_req
 * 8: w_timer1_dma_req
 * 7: i2c3_spi1_dma_sel(XH_SYS->MON[16], default=0)? i2c3_dma_rx : spi_mst1_dma_rx;
 * 6: i2c3_spi1_dma_sel(XH_SYS->MON[16], default=0)? i2c3_dma_tx : spi_mst1_dma_tx;
...
typedef enum
{
    ...
    I2C0_RX_DMA_ID               = 3,
#else
    SPI_MST0_TX_DMA_ID           = 2,
    SPI_MST0_RX_DMA_ID           = 3,
#endif
    TIMER0_DMA_ID                = 4,
    TIMER1_DMA_ID                = 5,
    TIMER2_DMA_ID                = 6,
    ADC_DMA_ID                   = 7,
    I2S_TX_DMA_ID                = 8, // XH98xx
    I2S_RX_DMA_ID                = 9, // XH98xx
 ...
 */
typedef XH_DMA_CH_Type dma_block_t;
...
typedef struct
{
    ...
    uint32_t   block_num;
    /// Link list item block length, MAX 4095*src_width/8 for XH98xx / (2**22 - 1)*src_width/8 for xh98xx, in bytes.
    uint32_t   block_len;
 ...
 */
/// for XH98xx's DesignWare DMA Controller -- the DW_ahb_dmac
typedef enum
{
...
typedef struct
{
    ...
    /// XH98xx:  max src_width_in_bytes * 4095;
    /// XH98xx: max src_width_in_bytes * (2**22 - 1).
 ...
 **/
__STATIC_INLINE uint32_t dma_get_src_addr(XH_DMA_CH_Type *ch)
{
    return ch->SrcAddr;
}
 ...
 **/
__STATIC_INLINE uint32_t dma_get_dst_addr(XH_DMA_CH_Type *ch)
{
    return ch->DstAddr;
}
 ...
 * @return current dest address
 **/
__STATIC_INLINE void dma_addr_inc_mode(XH_DMA_CH_Type *ch, uint32_t src_mode, uint32_t dst_mode)
{
    REGW(&ch->Ctrl, MASK_2REG(DMA_SRCADDRCTRL,src_mode, DMA_DSTADDRCTRL,dst_mode));
}
