...
enum BLE_GAP_EVTS
{
  ...
  BLE_GAP_EVT_AUTH_NUMERIC_COMPARISON_REQUEST,  /**< Request to display a passkey and provide Yes or No. @ref ble_gap_evt_passkey_display_t */
  ...
  BLE_GAP_EVT_PHY_UPDATE,                       /**< Connection PHY updated @ref ble_gap_evt_phy_update_t */
};
 ...
 * @{ */
#define BLE_ERROR_GAP_UUID_LIST_MISMATCH            (XH_GAP_ERR_BASE + 0x000)  /**< UUID list does not contain an integral number of UUIDs. */
#define BLE_ERROR_GAP_DISCOVERABLE_WITH_WHITELIST   (XH_GAP_ERR_BASE + 0x001)  /**< Use of Whitelist not permitted with discoverable advertising. */
#define BLE_ERROR_GAP_INVALID_BLE_ADDR              (XH_GAP_ERR_BASE + 0x002)  /**< The upper two bits of the address do not correspond to the specified address type. */
/**@} */
...
#define BLE_GAP_ADV_TYPE_ADV_LOW_DIRECT_IND 0x04   /**< Connectable low duty directed. */
/**@} */
...
/**
 * @brief BLE GAP init options
 */
typedef struct
{
  uint8_t role:2;             /**< BLE role to use, see @ref BLE_GAP_ROLES */
...
typedef struct
{
  ...
  uint8_t               sc      : 1;               /**< 4.2: Secure Connections required. */
  uint8_t               keypress: 1;               /**< 4.2: Keypress notification available. */
...
/**
 * @brief GAP set PHY preference
 */
typedef struct {
    uint8_t   tx_phys; /**< Connection Tx PHY preference. bit0 - 1M, bit1 - 2M, bit2 - Coded */
    uint8_t   rx_phys; /**< Connection Rx PHY preference. bit0 - 1M, bit1 - 2M, bit2 - Coded */
} ble_gap_conn_phy_t;
...
typedef struct
{
  ...
  /* FIXME: role is not defined in this event of s110, s120, but s130 */
...
typedef struct
{
  ble_gap_addr_t peer_addr;                     /**< Bluetooth address of the peer device. */
  int8_t         rssi;                          /**< Received Signal Strength Indication in dBm. */
  uint8_t        type;                          /**< See @ref BLE_GAP_ADV_TYPES. Only valid if the scan_rsp field is 0. */
...
/**@brief Event data for scan request report events. */
typedef struct
{
  int8_t                  rssi;              /**< Received Signal Strength Indication in dBm. */
  ble_gap_addr_t          peer_addr;         /**< Bluetooth address of the peer device. */
...
typedef struct
{
    ...
    uint8_t  tx_phy;                            /**<  Tx PHY in use, 0x01 - 1M, 0x02 - 2M, 0x03 - Coded */
    uint8_t  rx_phy;                            /**<  Rx PHY in use, 0x01 - 1M, 0x02 - 2M, 0x03 - Coded */
...
typedef struct
{
  ...
  union                                                     /**< union alternative identified by evt_id in enclosing struct. */
  {
    ...
    ble_gap_evt_phy_update_t                  phy_update;                   /**< LL PHY updated. */
 ...
 *        Used with @ref xh_ble_opt_get to get the current channel map
 *        or @ref xh_ble_opt_set to set a new channel map. When setting the
 ...
 *
 * @retval ::XH_SUCCESS Get or set successful.
 * @retval ::XH_ERROR_BUSY Channel map was set again before enough time had passed.
 ...
 * @retval ::XH_ERROR_NOT_SUPPORTED Returned by xh_ble_opt_set in peripheral-only SoftDevices.
 *
 */
typedef struct
{
 ...
 *
 *        Used with @ref xh_ble_opt_set to set the local connection latency. The
 *        @ref xh_ble_opt_get is not supported for this option, but the actual
 ...
 *
 * @retval ::XH_SUCCESS Set successfully.
 * @retval ::XH_ERROR_NOT_SUPPORTED Get is not supported.
 ...
 *        xh_ble_opt_set to make the SoftDevice use a pre-programmed passkey for authentication
 ...
 *
 *        This structure is used with both @ref xh_ble_opt_set (as input) and with
 *        @ref xh_ble_opt_get (as output).
 *
 ...
 *        @ref xh_ble_gap_address_set with the same type again. 
 *
 * @note  If the IRK is updated, the new IRK becomes the one to be distributed in all
 *        bonding procedures performed after @ref xh_ble_opt_set returns.
 *
 * @retval ::XH_SUCCESS Set or read successfully.
 * @retval ::XH_ERROR_INVALID_ADDR The pointer to IRK storage is invalid.
 */
typedef struct
{
 ...
 *
 *        This can be used with @ref xh_ble_opt_set to make the SoftDevice send
 ...
 *
 *  @retval ::XH_SUCCESS Set successfully.
 *  @retval ::XH_ERROR_INVALID_STATE When advertising is ongoing while the option is set.
 */
typedef struct
{
 ...
 *
 *        This can be used with @ref xh_ble_opt_set to enable and disable
 ...
 *
 *  @retval ::XH_SUCCESS Set successfully.
 *  @retval ::XH_ERROR_INVALID_STATE When connection creation is ongoing while mode 1 is set.
 */
typedef struct
{
 ...
 *
 * @retval ::XH_SUCCESS Address successfully set.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameters.
 * @retval ::BLE_ERROR_GAP_INVALID_BLE_ADDR Invalid address.
 * @retval ::XH_ERROR_BUSY The stack is busy, process pending events and retry.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 */
uint32_t xh_ble_gap_address_set(uint8_t addr_cycle_mode, const ble_gap_addr_t *p_addr);
 ...
 *
 * @retval ::XH_SUCCESS Address successfully retrieved.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 */
uint32_t xh_ble_gap_address_get(ble_gap_addr_t *p_addr);
 ...
 *
 * @retval ::XH_SUCCESS Advertising data successfully updated or cleared.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_FLAGS Invalid combination of advertising flags supplied.
 * @retval ::XH_ERROR_INVALID_DATA Invalid data type(s) supplied, check the advertising data format specification.
 * @retval ::XH_ERROR_INVALID_LENGTH Invalid data length(s) supplied.
 * @retval ::XH_ERROR_NOT_SUPPORTED Unsupported data type.
 * @retval ::BLE_ERROR_GAP_UUID_LIST_MISMATCH Invalid UUID list supplied.
 */
uint32_t xh_ble_gap_adv_data_set(uint8_t const *p_data, uint8_t dlen, uint8_t const *p_sr_data, uint8_t srdlen);
 ...
 *
 * @retval ::XH_SUCCESS The BLE stack has started advertising.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied, check the accepted ranges and limits.
 ...
 * @retval ::XH_ERROR_BUSY The stack is busy, process pending events and retry.
 */
uint32_t xh_ble_gap_adv_start(ble_gap_adv_params_t const *p_adv_params);


/**@brief Stop advertising (GAP Discoverable, Connectable modes, Broadcast Procedure).
 *
 * @retval ::XH_SUCCESS The BLE stack has stopped advertising.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation (most probably not in advertising state).
 */
uint32_t xh_ble_gap_adv_stop(void);
 ...
 *
 * @retval ::XH_SUCCESS The Connection Update procedure has been started successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied, check parameter limits and constraints.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::XH_ERROR_BUSY Procedure already in progress or not allowed at this time, process pending events and wait for pending procedures to complete and retry.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 * @retval ::XH_ERROR_NO_MEM Not enough memory to complete operation.
 */
uint32_t xh_ble_gap_conn_param_update(uint16_t conn_handle, ble_gap_conn_params_t const *p_conn_params);
 ...
 *
 * @retval ::XH_SUCCESS The disconnection procedure has been started successfully.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation (disconnection is already in progress).
 */
uint32_t xh_ble_gap_disconnect(uint16_t conn_handle, uint8_t hci_status_code);
 ...
 *
 * @retval ::XH_SUCCESS Successfully changed the transmit power.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 */
uint32_t xh_ble_gap_tx_power_set(int8_t tx_power);
 ...
 *
 * @retval ::XH_SUCCESS  Appearance value set successfully.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 */
uint32_t xh_ble_gap_appearance_set(uint16_t appearance);
 ...
 *
 * @retval ::XH_SUCCESS Appearance value retrieved successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 */
uint32_t xh_ble_gap_appearance_get(uint16_t *p_appearance);
 ...
 *
 * @retval ::XH_SUCCESS Peripheral Preferred Connection Parameters set successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 */
uint32_t xh_ble_gap_ppcp_set(ble_gap_conn_params_t const *p_conn_params);
 ...
 *
 * @retval ::XH_SUCCESS Peripheral Preferred Connection Parameters retrieved successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 */
uint32_t xh_ble_gap_ppcp_get(ble_gap_conn_params_t *p_conn_params);
 ...
 *
 * @retval ::XH_SUCCESS GAP device name and permissions set successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_DATA_SIZE Invalid data size(s) supplied.
 */
uint32_t xh_ble_gap_device_name_set(ble_gap_conn_sec_mode_t const *p_write_perm, uint8_t const *p_dev_name, uint16_t len);
 ...
 *
 * @retval ::XH_SUCCESS GAP device name retrieved successfully.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_DATA_SIZE Invalid data size(s) supplied.
 */
uint32_t xh_ble_gap_device_name_get(uint8_t *p_dev_name, uint16_t *p_len);
 ...
 *
 *
 * @retval ::XH_SUCCESS Successfully initiated authentication procedure.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::XH_ERROR_BUSY The stack is busy, process pending events and retry.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 * @retval ::XH_ERROR_TIMEOUT A SMP timout has occured, and further SMP operations on this link is prohibited.
 */
uint32_t xh_ble_gap_authenticate(uint16_t conn_handle, ble_gap_sec_params_t const *p_sec_params);
 ...
 *                         already been provided during a previous call to @ref xh_ble_gap_authenticate.
 ...
 *
 * @details This function is only used to reply to a @ref BLE_GAP_EVT_SEC_PARAMS_REQUEST, calling it at other times will result in an @ref XH_ERROR_INVALID_STATE.
 ...
 *
 * @retval ::XH_SUCCESS Successfully accepted security parameter from the application.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_sec_params_reply(uint16_t conn_handle, uint8_t sec_status, ble_gap_sec_params_t const *p_sec_params, ble_gap_sec_keyset_t const *p_sec_keyset);
 ...
 *
 * @details This function is only used to reply to a @ref BLE_GAP_EVT_AUTH_KEY_REQUEST, calling it at other times will result in an @ref XH_ERROR_INVALID_STATE.
 * @note    If the call returns an error code, the request is still pending, and the reply call may be repeated with corrected parameters.
 *
 * @retval ::XH_SUCCESS Authentication key successfully set.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_auth_key_reply(uint16_t conn_handle, uint8_t key_type, uint8_t const *p_key);


/**@brief deprecated.
 */
uint32_t xh_ble_gap_auth_yesno_reply(uint16_t conn_handle, bool accept);
 ...
 * @retval ::XH_ERROR_BUSY Procedure already in progress or not allowed at this time, wait for pending procedures to complete and retry.
 */
uint32_t xh_ble_gap_encrypt(uint16_t conn_handle, ble_gap_master_id_t const *p_master_id, ble_gap_enc_info_t const *p_enc_info);
 ...
 *
 * @details This function is only used to reply to a @ref BLE_GAP_EVT_SEC_INFO_REQUEST, calling it at other times will result in @ref XH_ERROR_INVALID_STATE.
 * @note    If the call returns an error code, the request is still pending, and the reply call may be repeated with corrected parameters.
 * @note    Data signing is not yet supported, and p_sign_info must therefore be NULL.
 *
 * @retval ::XH_SUCCESS Successfully accepted security information.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_sec_info_reply(uint16_t conn_handle, ble_gap_enc_info_t const *p_enc_info, ble_gap_irk_t const *p_id_info, ble_gap_sign_info_t const *p_sign_info);
 ...
 *
 * @retval ::XH_SUCCESS Current connection security successfully retrieved.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_conn_sec_get(uint16_t conn_handle, ble_gap_conn_sec_t *p_conn_sec);


/**@brief Set PHY preferences.
 *
 * @param[in]  conn_handle Connection handle.
 * @param[out] p_conn_phy  Pointer to a @ref ble_gap_conn_phy_t structure to be filled in.
 *
 * @retval ::XH_SUCCESS Current connection security successfully retrieved.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_set_phy(uint16_t conn_handle, ble_gap_conn_phy_t *p_conn_phy);

/**@brief Set Connection data length
 *
 * @param[in]  conn_handle Connection handle.
 * @param[in]  max_tx_octets Max TX octets
 * @param[in]  max_tx_time   Max Tx Time
 *
 * @retval ::XH_SUCCESS Current connection security successfully retrieved.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_set_data_length(uint16_t conn_handle, uint16_t max_tx_octets, uint16_t max_tx_time);

/**@brief Start reporting the received signal strength to the application. 
 *
 * A new event is reported whenever the RSSI value changes, until @ref xh_ble_gap_rssi_stop is called.
 *
 * @param[in] conn_handle        Connection handle.
 ...
 *
 * @retval ::XH_SUCCESS                   Successfully activated RSSI reporting.
 * @retval ::XH_ERROR_INVALID_STATE       Disconnection in progress. Invalid state to perform operation.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_rssi_start(uint16_t conn_handle, uint8_t threshold_dbm, uint8_t skip_count);
 ...
 * may be reported after @ref xh_ble_gap_rssi_stop has been called.
 *
 * @param[in] conn_handle Connection handle.
 *
 * @retval ::XH_SUCCESS                   Successfully deactivated RSSI reporting.
 * @retval ::XH_ERROR_INVALID_STATE       Invalid state to perform operation.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 */
uint32_t xh_ble_gap_rssi_stop(uint16_t conn_handle);
 ...
 *
 * @ref xh_ble_gap_rssi_start must be called to start reporting RSSI before using this function. @ref XH_ERROR_NOT_FOUND
 * will be returned until RSSI was sampled for the first time after calling @ref xh_ble_gap_rssi_start.
 *
 * @retval ::XH_SUCCESS                   Successfully read the RSSI.
 * @retval ::XH_ERROR_NOT_FOUND           No sample is available.
 * @retval ::XH_ERROR_INVALID_ADDR        Invalid pointer supplied.
 * @retval ::BLE_ERROR_INVALID_CONN_HANDLE Invalid connection handle supplied.
 * @retval ::XH_ERROR_INVALID_STATE       RSSI reporting is not ongoing, or disconnection in progress.
 */
uint32_t xh_ble_gap_rssi_get(uint16_t conn_handle, int8_t *p_rssi);
 ...
 *
 * @retval ::XH_SUCCESS Successfully initiated scanning procedure.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_BUSY The stack is busy, process pending events and retry.
 * @retval ::XH_ERROR_NOT_SUPPORTED A selected feature is not supported (selective scanning).
 */
uint32_t xh_ble_gap_scan_start(ble_gap_scan_params_t const *p_scan_params);


/**@brief Stop scanning (GAP Discovery procedure, Observer Procedure).
 *
 * @retval ::XH_SUCCESS Successfully stopped scanning procedure.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation (most probably not in scanning state).
 */
uint32_t xh_ble_gap_scan_stop(void);
 ...
 *
 * @retval ::XH_SUCCESS Successfully initiated connection procedure.
 * @retval ::XH_ERROR_INVALID_ADDR Invalid parameter(s) pointer supplied.
 * @retval ::XH_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 * @retval ::BLE_ERROR_GAP_INVALID_BLE_ADDR Invalid Peer address.
 * @retval ::XH_ERROR_NO_MEM limit of available connections reached.
 * @retval ::XH_ERROR_BUSY The stack is busy, process pending events and retry.
*/
uint32_t xh_ble_gap_connect(ble_gap_addr_t const *p_peer_addr, ble_gap_scan_params_t const *p_scan_params, ble_gap_conn_params_t const *p_conn_params);


/**@brief Cancel a connection establishment.
 *
 * @retval ::XH_SUCCESS Successfully cancelled an ongoing connection procedure.
 * @retval ::XH_ERROR_INVALID_STATE Invalid state to perform operation.
 */
uint32_t xh_ble_gap_connect_cancel(void);
