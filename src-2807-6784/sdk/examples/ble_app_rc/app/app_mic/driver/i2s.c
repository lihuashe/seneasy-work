...
/**
 * @brief i2s config tx no ws and sclk
 *
 * @param[in] config  configuration @ref using_dma
 *
 * @return None
 **/
void i2s_config_tx_no_ws_and_sclk(i2s_config_t *config, bool bUse_dma)
{
    register_set1(&XH_PSO->I2S_CFG, CPM_I2S_TX_AHB_EN_MASK|CPM_I2S_TX_SOFT_RESET_MASK);
    XH_PSO_UPD_RDY();

    /*disable the i2s clock generation block*/
    XH_I2S->CER = 0x00;
    /*disable the i2s periperal*/
    XH_I2S->IER = 0x00;
    /*disable the i2s transmitter block*/
    XH_I2S->ITER = 0x00;
    /*disable the i2s transmitter channel 0*/
    XH_I2S->CHN_R0.TER = 0x00;
    /*flush the receiver block fifos and channel 0 fifos*/
    XH_I2S->TXFFR = 0x01;      /*self-clearing*/
    XH_I2S->CHN_R0.TFF = 0x01; /*self-clearing*/
    /*set audio data resolution, 16bit resolution, which is fixed*/
    XH_I2S->CHN_R0.TCR = 0x02;
    /*set the tx data available trigger level*/
    XH_I2S->CHN_R0.TFCR = 0x00;

    if(false == bUse_dma)
    {
        XH_I2S->CHN_R0.IMR &= ~(I2S_IMR_TXFOM|I2S_IMR_TXFEM);
    ...
    /*enable the i2s periperal*/
    //  XH_I2S_TX->IER = (1<<4); /*mono channel*/
    XH_I2S->IER = 0x00;          /*setero channel*/
}
 ...
 * @retval NOT_NULL: Current used DMA
 **/
XH_DMA_CH_Type *i2s_dma_config2_no_lli(XH_DMA_CH_Type *dma, const i2s_dma_config_t *config)
{
    if(dma == NULL)
        dma = dma_allocate();
...
/**
 * @brief i2s start tx no ws and sclk
 *
 * @return None
 **/
...
{
    XH_I2S->IER |= 0x01;
    /*enable the i2s transmitter block*/
    XH_I2S->ITER = 0x01;
    /*enable the i2s tx channel*/
    XH_I2S->CHN_R0.TER = 0x01;

    //codec_if_ctrl[4]=1'b0, ws and sclk input by gpio

    XH_AUDIO->IF_CTRL = 0x05;
}
...
/**
 * @brief i2s config rx no ws and sclk
 *
 * @return None
 **/
void i2s_config_rx_no_ws_and_sclk(i2s_bitwidth_t bitwidth, bool bUse_dma)
{
    register_set1(&XH_PSO->I2S_CFG, CPM_I2S_RX_AHB_EN_MASK|CPM_I2S_RX_SOFT_RESET_MASK);
    XH_PSO_UPD_RDY();
    ...
    /*disable the i2s periperal*/
    XH_I2S_RX->IER = 0x00;
    /*disable the i2s receive block*/
    XH_I2S_RX->IRER = 0x00;
    /*disable the i2s receive channel 0*/
    XH_I2S_RX->CHN_R0.RER = 0x00;
    /*flush the receiver block fifos and channel 0 fifos*/
    XH_I2S_RX->RXFFR = 0x01;      /*self-clearing*/
    XH_I2S_RX->CHN_R0.RFF = 0x01; /*self-clearing*/
    
    /*set audio data resolution*/
    XH_I2S_RX->CHN_R0.RCR = bitwidth;
    
    /*set the rx data available trigger level*/
    XH_I2S_RX->CHN_R0.RFCR = 0x00;
    /*mask tx fifo interrupt*/

    /*enable the i2s periperal*/
    //  XH_I2S_RX->IER = (1<<4); /*mono channel*/
    XH_I2S_RX->IER = 0x00;          /*setero channel*/
    
    if (bUse_dma) {
        /*disable tx interrupt*/
        XH_I2S_RX->CHN_R0.IMR |= I2S_IMR_TXFOM|I2S_IMR_TXFEM;
        NVIC_DisableIRQ(I2S_TX_IRQn);
    } else {
        /*enable rx interrupt*/
        XH_I2S_RX->CHN_R0.IMR &= ~(I2S_IMR_RXFOM|I2S_IMR_RXDAM);
...
void i2s_config_rx_gen_ws_and_sclk(i2s_config_t *config, bool bUse_dma)
{
    ...
    XH_AUDIO->CODEC_CLK_CTRL_1 = 0x0F; 
    ...
    /*set audio data resolution*/
    //__i2s_set_rxsamplewidth(XH_I2S_RX,config.sample_width);
    XH_I2S_RX->CHN_R0.RCR = config->sample_width;
...
/**
 * @brief i2s start rx gen ws and sclk
 *
 * @return None
 **/
...
{
    XH_I2S_RX->IER |= 0x01;
    /*enable the i2s receiver block*/
    XH_I2S_RX->IRER = 0x01;
    /*enable the i2s rx channel*/
    XH_I2S_RX->CHN_R0.RER = 0x01;
    XH_AUDIO->IF_CTRL = 0x26;
}

/**
 * @brief i2s start rx no ws and sclk
 *
 * @return None
 **/
...
{
    XH_I2S_RX->IER |= 0x01;
    /*enable the i2s receive block*/
    XH_I2S_RX->IRER = 0x01;
    /*enable the i2s rx channel*/
    XH_I2S_RX->CHN_R0.RER = 0x01;

    //codec_if_ctrl[4]=1'b0, ws and sclk input by gpio
    XH_AUDIO->IF_CTRL = 0x06;
}
