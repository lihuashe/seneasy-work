 ...
 *                                  Macros
 *============================================================================*/
#define TEST_DEVICE_TYPE 0x41  /* bee3 DUT device type */
...
const T_UART_TEST_PROTOCOL uart_test_func_map[UART_TEST_SUPPORT_NUM] =
{
    ...
    {SET_VOICE_CONFIG_CMD, 14, uart_test_set_voice_config},
...
static void uart_test_set_voice_config(void *p_data)
{
    ...
    UART_DBG_BUFFER(MODULE_APP, LEVEL_INFO,
                    "[uart_test_set_voice0_config] voice config %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
                    14,
                    ...
                    codec_ch0_boost_gain, codec_ch0_zero_det_timeout);
...
#if SUPPORT_VOICE_FEATURE
    UART_DBG_BUFFER(MODULE_APP, LEVEL_INFO,
                    "[uart_test_get_voice0_config] codec config %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
                    14,
...
static void uart_test_enter_hci_download_mode(void *p_data)
{
    ...
    WDG_SystemReset(RESET_ALL_EXCEPT_AON, UART_CMD_RESET);
#else
...
void uart_test_set_codec_eq_config(void *p_data)
{
    ...
    if ((eq_channel_index >= SW_EQ_MAX_STAGE)
    ...
    else
    {
        ...
        {
            SW_EQ_PARAMS_TABLE[eq_channel_index][0] = EQ_CH_Cmd_DISABLE;
            SW_EQ_PARAMS_TABLE[eq_channel_index][1] = 0;
            SW_EQ_PARAMS_TABLE[eq_channel_index][2] = 0;
            SW_EQ_PARAMS_TABLE[eq_channel_index][3] = 0;
            SW_EQ_PARAMS_TABLE[eq_channel_index][4] = 0;
            SW_EQ_PARAMS_TABLE[eq_channel_index][5] = 0;
        }
        else if (eq_status == UART_TEST_CODEC_EQ_STATUS_ENABLE)
        {
            SW_EQ_PARAMS_TABLE[eq_channel_index][0] = EQ_CH_Cmd_ENABLE;
            SW_EQ_PARAMS_TABLE[eq_channel_index][1] = eq_coef_H0;
            SW_EQ_PARAMS_TABLE[eq_channel_index][2] = eq_coef_B1;
            SW_EQ_PARAMS_TABLE[eq_channel_index][3] = eq_coef_B2;
            SW_EQ_PARAMS_TABLE[eq_channel_index][4] = eq_coef_A1;
            SW_EQ_PARAMS_TABLE[eq_channel_index][5] = eq_coef_A2;
        }
        result = true;
    }
    ...
    if (result == true)
    {
        data_uart_cmd_response(SET_CODEC_EQ_CONFIG_CMD, UART_TEST_SUCCESS, NULL, 0);
    }
    else
    {
        data_uart_cmd_response(SET_CODEC_EQ_CONFIG_CMD, UART_TEST_ERROR, NULL, 0);
    }
}
...
void uart_test_get_codec_eq_config(void *p_data)
{
    ...
    UART_DBG_BUFFER(MODULE_APP, LEVEL_INFO, "[uart_test_get_codec_eq_config] %d %d %d %d %d %d %d", 7,
                    eq_channel_index,
                    SW_EQ_PARAMS_TABLE[eq_channel_index][0],
                    SW_EQ_PARAMS_TABLE[eq_channel_index][1],
                    SW_EQ_PARAMS_TABLE[eq_channel_index][2],
                    SW_EQ_PARAMS_TABLE[eq_channel_index][3],
                    SW_EQ_PARAMS_TABLE[eq_channel_index][4],
                    SW_EQ_PARAMS_TABLE[eq_channel_index][5]
                   );

    if (SW_EQ_PARAMS_TABLE[eq_channel_index][0] == EQ_CH_Cmd_DISABLE)
    {
        eq_config_buff[0] = 0;
    }
    else if (SW_EQ_PARAMS_TABLE[eq_channel_index][0] ==  EQ_CH_Cmd_ENABLE)
    {
        eq_config_buff[0] = 1;
    }
    ...
    eq_config_buff[2] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][1] >> 8);
    eq_config_buff[3] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][1] >> 16);
    eq_config_buff[4] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][1] >> 24);

    eq_config_buff[5] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][2]);
    eq_config_buff[6] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][2] >> 8);
    eq_config_buff[7] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][2] >> 16);
    eq_config_buff[8] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][2] >> 24);

    eq_config_buff[9] = (uint8_t)(
                            SW_EQ_PARAMS_TABLE[eq_channel_index][3]);
    eq_config_buff[10] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][3] >> 8);
    eq_config_buff[11] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][3] >> 16);
    eq_config_buff[12] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][3] >> 24);

    eq_config_buff[13] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][4]);
    eq_config_buff[14] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][4] >> 8);
    eq_config_buff[15] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][4] >> 16);
    eq_config_buff[16] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][4] >> 24);

    eq_config_buff[17] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][5]);
    eq_config_buff[18] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][5] >> 8);
    eq_config_buff[19] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][5] >> 16);
    eq_config_buff[20] = (uint8_t)(
                             SW_EQ_PARAMS_TABLE[eq_channel_index][5] >> 24);

    data_uart_cmd_response(GET_CODEC_EQ_CONFIG_CMD, UART_TEST_SUCCESS, eq_config_buff, 21);
}
#endif
