    ...
    case GAP_CONN_STATE_CONNECTED:
        {
            ...
            os_timer_stop(&wait4_conn_timer_handle);
            os_timer_start(&image_transfer_timer_handle);
            if (timeout_value_ctittv && (OTP->ota_timeout_ctittv != 0xFF))
            {
                os_timer_start(&ctittv_timer_handle);
            }
        }
        break;
        ...
        case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
            {
                ...
                if (DFU_WRITE_ATTR_EXIT == dfu_write_opcode)
                {
                        ...
                        case DFU_OPCODE_START_DFU:
                            {
                                ...
                                if (timeout_value_wait_packet)
                                {
                                    os_timer_start(&wait_packet_timer_handle);
                                }
#endif
                            }
                            break;
                        ...
                        case DFU_OPCODE_VALID_FW:
                            {
                                ...
                                if (timeout_value_wait_packet)
                                {
                                    os_timer_stop(&wait_packet_timer_handle);
                                }
#endif
                    ...
                    else if (p_dfu_cb_data->msg_data.write.write_attrib_index == INDEX_DFU_PACKET_VALUE)
                    {
#if OTA_TIMEOUT_WAIT_PACKET
                        if (timeout_value_wait_packet)
                        {
                            os_timer_restart(&wait_packet_timer_handle, timeout_value_wait_packet);
                        }
#endif
                    }
                    else
                ...
                else if (DFU_WRITE_START == dfu_write_opcode)
                {
                    ...
                    if ((T_IMG_ID)p_header->image_id >= OTA && (T_IMG_ID)p_header->image_id < IMAGE_MAX &&
                        (p_header->payload_len + IMG_HEADER_SIZE) < get_temp_ota_bank_size_by_img_id((
                                    T_IMG_ID)p_header->image_id))
                    {
                        uint32_t total_period = timeout_value_total * ((p_header->payload_len + IMG_HEADER_SIZE) / 102400 +
                                                                       1);
                        os_timer_restart(&total_timer_handle, total_period);
                        uint32_t transfer_period = timeout_value_image_transfer * ((p_header->payload_len +
                                                                                    IMG_HEADER_SIZE) / 102400 +
                                                                                   1);
                    ...
                    if (timeout_value_ctittv)
                    {
                        os_timer_stop(&ctittv_timer_handle);
                    }
