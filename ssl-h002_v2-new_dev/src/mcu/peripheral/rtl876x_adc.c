...
void ADC_Init(ADC_TypeDef *ADCx, ADC_InitTypeDef *ADC_InitStruct)
{
    ...
    btaon_fast_write_safe(0x116, reg_value | BIT(10));

    /* vref_sel = 0.85V */
    reg_value = btaon_fast_read_safe(0x116);
    btaon_fast_write_safe(0x116, reg_value | BIT(5) | BIT(4));
...
void ADC_Cmd(ADC_TypeDef *ADCx, uint8_t adcMode, FunctionalState NewState)
{
    ...
    if (NewState == ENABLE)
    {
        ...
        btaon_fast_write_safe(0x113, reg_value & ~BIT2);
        /* In case manual mode */
...
void ADC_BypassCmd(uint8_t ChannelNum, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        btaon_fast_write_safe(0x118, reg_value);
    }
    else
    {
        reg_value = btaon_fast_read_safe(0x118);
