...
/**
  ...
  * @retval None
  */
void RTC_SelectSrcToGpioInput(RTC_32K_GPIO_IN gpio)
{
    // SEL_GPIO_32K_rtc = 1, en_gpio_32k = 1
    btaon_fast_update_safe(AON_FAST_REG_REG2X_AON_SYS, BIT10 | BIT11, (0x3 << 10));

    if (gpio == RTC_32K_XI)
    {
        // rtc_in_selsy = 0x1 (PAD_32K_XI)
        btaon_fast_update_safe(AON_FAST_REG_REG2X_AON_SYS, BIT8 | BIT9, (0x1 << 8));
    }
    else
    {
        // rtc_in_selsy = 0x2 (PAD_P2_4)
        btaon_fast_update_safe(AON_FAST_REG_REG2X_AON_SYS, BIT8 | BIT9, (0x2 << 8));
    }
}
