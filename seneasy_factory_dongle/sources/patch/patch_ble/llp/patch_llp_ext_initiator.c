...
typedef enum
{
    LLP_EXT_INITIATOR_SUB_STATE_IDLE = 0,
    LLP_EXT_INITIATOR_SUB_STATE_CONTINUE_RX,
    LLP_EXT_INITIATOR_SUB_STATE_TX,
    LLP_EXT_INITIATOR_SUB_STATE_RX,
}
EN_LLP_EXT_INITIATOR_SUB_STATE_T;

extern volatile EN_LLP_EXT_INITIATOR_SUB_STATE_T g_llpExtInitiatorSubState;
...
EN_LLP_ERR_T patch_llp_ext_initiator_fragment_start(stLlpExtInitFragmentParam_t* pstParam)
{
    ...
    err = rom_llp_hw_set_work_time_us(LLP_FRAGMENT_TYPE_EXT_INIT, pstParam->u32WorkTimeUs);
    if(LLP_ERR_NONE != err)
    {
        return err;
    }
    ...
    //set mode
    g_llpExtInitiatorSubState = LLP_EXT_INITIATOR_SUB_STATE_IDLE;
