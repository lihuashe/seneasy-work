...
kb_k_mp_t *    kb_p_map[4] = {(kb_k_mp_t * )kb_map_normal, (kb_k_mp_t * )kb_map_num, (kb_k_mp_t * )kb_map_fn, (kb_k_mp_t * )kb_map_fn};
...
u32 kb_scan_key_value (int numlock_status, int read_key,unsigned char * gpio)
{
       ...
       //if(key_changed) printf("key_changed === 1\r\n");
...
u8 kb_powerdet(void)
{
    ...
    for(i=0;i<2;i++)
    {
        gpio_write(drive_pins[1], KB_LINE_HIGH_VALID); // watch out the KB_LINE_HIGH_VALID polarity, it will affect deep IO wakeup
