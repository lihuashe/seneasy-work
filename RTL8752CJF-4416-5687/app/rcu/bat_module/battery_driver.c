...
void update_vbat_value(uint16_t src_data)
{
    ...
    if (vbat_data.vbat_value >= 3200)/*>3.2V, 100%*/
    {
        vbat_data.vbat_level = 100;
    }
    else if (vbat_data.vbat_value >= 3100)
    {
        vbat_data.vbat_level = 97;
    }
    else if (vbat_data.vbat_value >= 3000)
    {
        vbat_data.vbat_level = 79;
    }
    else if (vbat_data.vbat_value >= 2900)
    {
        vbat_data.vbat_level = 69;
    }
    else if (vbat_data.vbat_value >= 2800)
    {
        vbat_data.vbat_level = 59;
    }
    else if (vbat_data.vbat_value >= 2700)
    {
        vbat_data.vbat_level = 49;
    }
    else if (vbat_data.vbat_value >= 2600)
    {
        vbat_data.vbat_level = 39;
    }
    else if (vbat_data.vbat_value >= 2500)
    {
        vbat_data.vbat_level = 29;
    }
    else if (vbat_data.vbat_value >= 2400)
    {
        vbat_data.vbat_level = 24;
    }
    else if (vbat_data.vbat_value >= 2370)
    {
        vbat_data.vbat_level = 22;
    }
    else if (vbat_data.vbat_value >= 2350)
    {
        vbat_data.vbat_level = 20;
    }
    else if (vbat_data.vbat_value >= 2300)
    {
        vbat_data.vbat_level = 13;
    }
    else if (vbat_data.vbat_value >= 2200) 
    {
        vbat_data.vbat_level = 4;
    }
    else/*<2.2V, <15%*/
    {
        vbat_data.vbat_level = 0;
    }
...
{
    LED_BLINK(LED_1, LED_TYPE_BLINK_LOW_POWER, 3);
}
#endif
...
{
    static uint8_t pair_success_battary_flag = 0;
    ...
    case IO_MSG_TYPE_BAT_DETECT:
        {
            uint16_t value, level = 0;
            if(pair_success_battary_flag == 1){
                battery_get_value(&level, &value);
                if (value <= BAT_ADC_DETECT_THRESHOLD){
            ...
            {
                APP_PRINT_INFO1("[BAT] vbat_data.vbat_detect_by_cnt = %d",vbat_data.vbat_detect_by_cnt);
                if (++vbat_data.vbat_detect_by_cnt % VBAT_DETECT_KEY_CNT)
                {
                    return;
                }
                vbat_data.vbat_detect_by_cnt = 5;
                battery_get_value(&level, &value);
                if (value >= BAT_LOW_POWER_THRESHOLD && value <= BAT_ADC_DETECT_THRESHOLD){
#if BAT_LOW_POWER_INDICATE
                    APP_PRINT_INFO1("[BAT] app_global_data.adv_type = %d",app_global_data.adv_type);
