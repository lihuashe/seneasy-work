...
bool single_tone_test_flag = false;
/*============================================================================*
 *                              Local Variables
 *============================================================================*/
...
bool mp_test_enable_test_mode_flag(void)
{
    ...
    uint32_t test_mode_value = MP_TEST_MODE_FLG_ENABLE_VALUE;

    result = ftl_save(&test_mode_value, MP_TEST_FTL_PARAMS_TEST_MODE_FLG_OFFSET,
...
{
    uint32_t result = false;
    uint32_t test_mode_value = MP_TEST_MODE_FLG_DISABLE_VALUE;
...
{
    uint32_t ftl_res = 0;
    uint32_t test_mode_value = 0;

    ftl_res = ftl_load(&test_mode_value, MP_TEST_FTL_PARAMS_TEST_MODE_FLG_OFFSET,
                       MP_TEST_FTL_PARAMS_TEST_MODE_FLG_LEN);
    ...
    if (ftl_res == FTL_READ_ERROR_READ_NOT_FOUND)
    {
        ...
        ftl_load(&test_mode_value, MP_TEST_FTL_PARAMS_TEST_MODE_FLG_OFFSET,
                 MP_TEST_FTL_PARAMS_TEST_MODE_FLG_LEN);
    }
    ...
    return (test_mode_value == MP_TEST_MODE_FLG_ENABLE_VALUE);
}
...
/******************************************************************
* @brief    MP Test handle two keys scenario
*/
uint32_t mp_test_handle_two_keys_scenario(T_KEY_INDEX_DEF key_index_1, T_KEY_INDEX_DEF key_index_2)
{
    uint32_t combine_key_result = INVALID_COMBINE_KEYS_BIT_MASK;
    bool is_test_mode_en = mp_test_is_test_mode_flag_en();

    if (is_test_mode_en == false)
    {
        combine_key_result =  INVALID_COMBINE_KEYS_BIT_MASK;
    }
#if MP_TEST_MODE_SUPPORT_HCI_UART_TEST
    else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_VOLUME_UP)) ||
             ((key_index_1 == VK_VOLUME_UP) && (key_index_2 == VK_POWER)))
    {
        combine_key_result = HCI_UART_TEST_COMBINE_KEYS_BIT_MASK;
    }
#endif
#if MP_TEST_MODE_SUPPORT_DATA_UART_TEST
    else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_VOLUME_DOWN)) ||
             ((key_index_1 == VK_VOLUME_DOWN) && (key_index_2 == VK_POWER)))
    {
        combine_key_result = DATA_UART_TEST_COMBINE_KEYS_BIT_MASK;
    }
#endif
#if MP_TEST_MODE_SUPPORT_SINGLE_TONE_TEST
    // else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_EXIT)) ||
    //          ((key_index_1 == VK_EXIT) && (key_index_2 == VK_POWER)))
    // {
    //     combine_key_result = SINGLE_TONE_TEST_COMBINE_KEYS_BIT_MASK;
    // }
#endif
#if MP_TEST_MODE_SUPPORT_FAST_PAIR_TEST
    else if (app_global_data.test_mode != AUTO_PAIR_WITH_FIX_ADDR_MODE)
    {
        if (((key_index_1 == VK_POWER) && (key_index_2 == VK_UP)) ||
            ((key_index_1 == VK_UP) && (key_index_2 == VK_POWER)))
        {
            combine_key_result = FAST_PAIR_1_COMBINE_KEYS_BIT_MASK;
        }
        else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_DOWN)) ||
                 ((key_index_1 == VK_DOWN) && (key_index_2 == VK_POWER)))
        {
            combine_key_result = FAST_PAIR_2_COMBINE_KEYS_BIT_MASK;
        }
        else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_LEFT)) ||
                 ((key_index_1 == VK_LEFT) && (key_index_2 == VK_POWER)))
        {
            combine_key_result = FAST_PAIR_3_COMBINE_KEYS_BIT_MASK;
        }
        else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_RIGHT)) ||
                 ((key_index_1 == VK_RIGHT) && (key_index_2 == VK_POWER)))
        {
            combine_key_result = FAST_PAIR_4_COMBINE_KEYS_BIT_MASK;
        }
        else if (((key_index_1 == VK_POWER) && (key_index_2 == VK_ENTER)) ||
                 ((key_index_1 == VK_ENTER) && (key_index_2 == VK_POWER)))
        {
            combine_key_result = FAST_PAIR_5_COMBINE_KEYS_BIT_MASK;
        }
    }
#endif

    APP_PRINT_INFO2("[mp_test_handle_two_keys_scenario] is_test_mode_en = %d, combine_key_result = 0x%04X",
                    is_test_mode_en, combine_key_result);

    return combine_key_result;
}
...
void mp_test_check_trig_gpio_status(void)
{
    ...
    uint8_t read_single_test_gpio_status;
    ...
    /* pad configuration */
    Pad_Config(MP_TEST_TRIG_PIN_1, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
               PAD_OUT_LOW);

    Pad_Config(ROW0, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);
    Pad_Config(ROW1, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);
    Pad_Config(ROW3, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);
    Pad_Config(ROW4, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);
    Pad_Config(ROW5, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);
    Pad_Config(ROW6, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,
            PAD_OUT_HIGH);

    Pad_Config(MP_TEST_TRIG_PIN_2, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE,
               PAD_OUT_HIGH);

    GPIO_InitTypeDef Gpio1_Struct;
    GPIO_StructInit(&Gpio1_Struct);
    Gpio1_Struct.GPIO_Pin = GPIO_GetPin(MP_TEST_TRIG_PIN_1);
    Gpio1_Struct.GPIO_Mode = GPIO_Mode_OUT;
    Gpio1_Struct.GPIO_ITCmd = DISABLE;
    GPIO_Init(&Gpio1_Struct);

    GPIO_InitTypeDef Gpio2_Struct;
    GPIO_StructInit(&Gpio2_Struct);
    Gpio2_Struct.GPIO_Pin = GPIO_GetPin(MP_TEST_TRIG_PIN_2);
    Gpio2_Struct.GPIO_Mode = GPIO_Mode_IN;
    Gpio2_Struct.GPIO_ITCmd = DISABLE;
    GPIO_Init(&Gpio1_Struct);

    read_single_test_gpio_status =  GPIO_ReadInputDataBit(GPIO_GetPin(MP_TEST_TRIG_PIN_2));
    ...
    Pad_Config(MP_TEST_TRIG_PIN_2, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
               PAD_OUT_HIGH);
    Pad_Config(ROW0, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    Pad_Config(ROW1, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    Pad_Config(ROW3, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    Pad_Config(ROW4, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    Pad_Config(ROW5, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    Pad_Config(ROW6, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_DOWN, PAD_OUT_DISABLE,
                PAD_OUT_HIGH);
    APP_PRINT_INFO1("[mp_test_check_trig_gpio_status] read_single_test_gpio_status = %d",read_single_test_gpio_status);
    if(read_single_test_gpio_status == 0)
    {
        DBG_DIRECT("[mp_test_check_trig_gpio_status] Switch to Syseneasy test mode!");
        single_tone_test_flag = true;
    }
