...
const led_event_stg led_event_arr[LED_TYPE_MAX] =
{
    ...
    {LED_TYPE_BLINK_KEY_PRESS_MODE,   LED_LOOP_BITS_KEY_PRESS_MODE,   LED_BIT_MAP_KEY_PRESS_MODE},
...
bool ambilight_check_dlps_allowed(void);
bool ambilight_check_dlps_allowed_flag = true;

bool ambilight_check_dlps_allowed(void)
{
    return ambilight_check_dlps_allowed_flag;
}

void ambilight_on(void)
{
   ambilight_check_dlps_allowed_flag = false;
}
void ambilight_off(void)
{
   ambilight_check_dlps_allowed_flag = true;
}
/**
...
void led_module_init(void)
{
    ...
    led_off(LED_1 & 0xFF);
    Pad_Config(POWER_EN, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_HIGH);
...
LED_RET_CAUSE led_blink_exit(uint16_t index, LED_TYPE type)
{
    ...
    /*start led*/
    if (LED_TYPE_IDLE != (led_arr[index >> 8].led_map & (1 << type)))
    {
#if LED_DEBUG
 ...
 * @note       none.
**/
void led_ctrl_timer_cb(TimerHandle_t pxTimer) DATA_RAM_FUNCTION;
void led_ctrl_timer_cb(TimerHandle_t pxTimer)
{
#if LED_DEBUG
...
/** Define pwm output pin.
  ...
  */
#define PWM_OUT_PIN             P1_0

#define PWM_TIMER_NUM           TIM1
#define PWM_OUT_PIN_PINMUX      timer_pwm1
...
void driver_pwm_init(void)
{
    ...
    /*Initialize TIM*/
    TIM_InitStruct.TIM_ClockSrc  = TIM_CLOCK_DIVIDER_1;
