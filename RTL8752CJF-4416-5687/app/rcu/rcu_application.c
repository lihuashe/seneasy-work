...
#define HW_VERSION      "S4416A4159-V"
#define FW_VERSION      "V" NAME2STR(1)"."NAME2STR(0)"."NAME2STR(4)

#elif (RCU_HD_PLATFORM_SEL == SRC_4416_5291)
#define HW_VERSION      "S4416A4159"
#define FW_VERSION      "V" NAME2STR(1)"."NAME2STR(1)"."NAME2STR(0)
#elif (RCU_HD_PLATFORM_SEL == SRC_3925_5879)
#define HW_VERSION      "S3925A4502"
#define FW_VERSION      "RC-3925V" NAME2STR(1)"."NAME2STR(0)"."NAME2STR(0)
#endif
    ...
    case STOP_ADV_REASON_IDLE:
        ...
        if (app_global_data.adv_type == ADV_DIRECT_HDC)
        {
            ...
            else
            {
...
#elif (RCU_HD_PLATFORM_SEL == SRC_4416_5291)
                else if (key_handle_global_data.reconn_key_index == MM_POWER)
#elif (RCU_HD_PLATFORM_SEL == SRC_3925_5879)
                else if (key_handle_global_data.reconn_key_index == MM_POWER)
#endif       
                {
                ...
                else if (key_handle_global_data.reconn_key_index == MM_AC_Search_Noconnect)
                {
                    key_handle_global_data.reconn_key_index = VK_NC;
                    ir_send_key_press_handle(MM_AC_Search_Noconnect, VK_NC, 1);
    ...
    case SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE:
        {
            ...
            {
#if (RCU_HD_PLATFORM_SEL == SRC_4416_5302)
                const uint8_t DISModelNumber[] = "SRC-4416-5687";
#elif (RCU_HD_PLATFORM_SEL == SRC_4416_5291)
                const uint8_t DISModelNumber[] = "SRC-4416-5219";
#elif (RCU_HD_PLATFORM_SEL == SRC_3925_5879)
                const uint8_t DISModelNumber[] = "SRC-3925-5879";
#endif                
            ...
            {
                const uint8_t DISSerialNumber[] = "20231225";
            ...
            {
                const uint8_t DISHardwareRev[] = HW_VERSION;
            ...
            {
                const uint8_t DISFirmwareRev[] = FW_VERSION;
            ...
            {
                const uint8_t DISSoftwareRev[] = FW_VERSION;
...
#if (RCU_HD_PLATFORM_SEL == SRC_4416_5302)
                uint8_t DISPnpID[DIS_PNP_ID_LENGTH] = {0x01, 0x5A, 0x1D, 0x9A, 0xC0, (uint8_t)version, (uint8_t)(version >> 8)}; //VID_005D&PID_0001?
                dis_set_parameter(DIS_PARAM_PNP_ID,
                                  sizeof(DISPnpID),
                                  DISPnpID);
#elif (RCU_HD_PLATFORM_SEL == SRC_4416_5291)
                uint8_t DISPnpID[DIS_PNP_ID_LENGTH] = {0x01, 0x5A, 0x1D, 0x80, 0xC0, (uint8_t)version, (uint8_t)(version >> 8)}; //VID_005D&PID_0001?
                dis_set_parameter(DIS_PARAM_PNP_ID,
                                  sizeof(DISPnpID),
                                  DISPnpID);
#elif (RCU_HD_PLATFORM_SEL == SRC_3925_5879)
                uint8_t DISPnpID[DIS_PNP_ID_LENGTH] = {0x01, 0x7A, 0x0B, 0x88, 0x14, (uint8_t)version, (uint8_t)(version >> 8)}; //VID_0B7A&PID_1488?
                dis_set_parameter(DIS_PARAM_PNP_ID,
                                  sizeof(DISPnpID),
                                  DISPnpID);
#endif
    ...
    case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
        {
            ...
            {
                APP_PRINT_INFO1("[app_vendor_srv_cb] msg_data.len = %d.",p_data->msg_data.write_msg.write_parameter.report_data.len);
                uint8_t i = 0;
                uint8_t receive_data[5];
                memcpy(receive_data, p_data->msg_data.write_msg.write_parameter.report_data.report,5);
                for(i = 0;i < 5;i ++){
                    APP_PRINT_INFO2("[app_vendor_srv_cb] receive_data[%d] =0x%x",i,receive_data[i]);
                }
                if(receive_data[0] == 0x54 && receive_data[1] == 0x65 && receive_data[2] == 0x73 &&\
                receive_data[3] == 0x74 && receive_data[4] == 0xF4){                                   
                    uint8_t mm_key_data[1] = {0x00};
                    mm_key_data[0] = 0x01;
                    server_send_data(0, app_global_data.hid_srv_id, GATT_SRV_HID_MM_KB_INPUT_INDEX, \
                                    mm_key_data, sizeof(mm_key_data), GATT_PDU_TYPE_NOTIFICATION);
                    APP_PRINT_INFO0("[app_vendor_srv_cb] Enable test mode flag success!");

                }    

                if(receive_data[0] == 0x54 && receive_data[1] == 0x65 && receive_data[2] == 0x73 &&\
                receive_data[3] == 0x74 && receive_data[4] == 0xF5){  
                    if (mp_test_clear_bond_info()){
...
/******************************************************************
 * @fn          app_smartrcu_srv_cb
 * @brief      smartrcu service callbacks are handled in this function.
 * @param    p_data  - pointer to callback data
 * @return     T_APP_RESULT
 */
static T_APP_RESULT app_smartrcu_srv_cb(T_SMARTRCU_CALLBACK_DATA *p_data)
{
    T_APP_RESULT cb_result = APP_RESULT_SUCCESS;

    // APP_PRINT_INFO1("[app_smartrcu_srv_cb] msg_type %d", p_data->msg_type);
     switch (p_data->msg_type)
    {
    case SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION:
        {
            if (p_data->msg_data.notification_indification_index == SMARTRCU_NOTIFY_ENABLE)
            {
                APP_PRINT_INFO0("[app_smartrcu_srv_cb] SMARTRCU_NOTIFY_ENABLE");
            }
            else if (p_data->msg_data.notification_indification_index == SMARTRCU_NOTIFY_DISABLE)
            {
                APP_PRINT_INFO0("[app_smartrcu_srv_cb] SMARTRCU_NOTIFY_DISABLE");
            }
        }
        break;
    case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
        {
            if (p_data->msg_data.write_msg.write_type == SMARTRCU_WRITE_DATA)
            {
                uint8_t command_data[40] = {};         
                // APP_PRINT_INFO2("[smartrcu_command_handle] len = %d,p_data->msg_data.write_msg.write_parameter.report_data.report: 0x %b",
                //         p_data->msg_data.write_msg.write_parameter.report_data.len,
                //         TRACE_BINARY(p_data->msg_data.write_msg.write_parameter.report_data.len, p_data->msg_data.write_msg.write_parameter.report_data.report));                
                memcpy(&command_data, p_data->msg_data.write_msg.write_parameter.report_data.report, p_data->msg_data.write_msg.write_parameter.report_data.len);
                smartrcu_command_handle((uint8_t *)&command_data);                
            }
            else
            {
                cb_result = APP_RESULT_APP_ERR;
                ...
                case ATV_TX_OPCODE_MIC_OPEN:
                    {
                        ...
                        LED_ON(LED_1);
                        os_timer_restart(&voice_overtime_led_off_timer, VOICE_OVERTIME_LED_OFF_TIMEOUT);
                        if (ATVV_VERSION_1_0 == atvv_global_data.app_support_version)
                        {
    ...
    case GAP_CONN_STATE_DISCONNECTED:
        {
            ...
            if (disc_cause == (HCI_ERR | HCI_ERR_REMOTE_USER_TERMINATE))
            {
                ...
                if (app_global_data.is_link_key_existed == false)
                {
                    ...
                    app_global_data.abnormal_discon_state = 1;
                    rcu_start_adv(ADV_UNDIRECT_RECONNECT);
                }
            }
    ...
    case GAP_AUTHEN_STATE_COMPLETE:
        {
            if (cause == GAP_CAUSE_SUCCESS)
            {
                ...
                // if(app_global_data.adv_type == ADV_UNDIRECT_PAIRING){
                //     os_timer_restart(&pair_success_led_exit_timer, PAIR_SUCCESS_TIMEOUT);
                // }
                rcu_bat_handle(IO_MSG_TYPE_BAT_FORCE_DETECT);
...
#elif (RCU_HD_PLATFORM_SEL == SRC_4416_5291)
                        if (key_handle_global_data.reconn_key_index != MM_POWER)
#elif (RCU_HD_PLATFORM_SEL == SRC_3925_5879)
                        if (key_handle_global_data.reconn_key_index != MM_POWER)
#endif  
                        {
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else if (service_id == app_global_data.smartrcu_srv_id)
    {
        app_result = app_smartrcu_srv_cb((T_SMARTRCU_CALLBACK_DATA *)p_data);
    }
