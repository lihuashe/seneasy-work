...
{
    2,                                                 //  HeaderLen;
    {PULSE_HIGH | 9000-100, PULSE_LOW | (4500 - 30)  },    //uint32_t HeaderContext[MAX_HEADDER_LEN];
    {PULSE_HIGH | 560-5, PULSE_LOW | (560 - 26)    },    //Log0[MAX_LOG_WAVFORM_SIZE];
    {PULSE_HIGH | 560-5, PULSE_LOW | (1690 - 30)   },    //Log1[MAX_LOG_WAVFORM_SIZE];
    PULSE_HIGH | 560,
    560,
    4
};
...
int irda_nec_encode(uint8_t address, uint8_t command, IRDA_BUF *pIrda)
{
    ...
    nec_Irdabuf->Code[0] = 0x1d;
    nec_Irdabuf->Code[1] = 0x02;
#else
