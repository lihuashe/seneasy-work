...
void IR_Learn_ConvertData(IR_LearnTypeDef *pIR_Packet)
{
    ...
    APP_PRINT_INFO2("[IR_Learn_ConvertData1] pIR_Packet->duty_cycle = %d,pIR_Packet->freq = %d",pIR_Packet->duty_cycle,pIR_Packet->freq);
...
void SmartRcu_ir_Decode(IR_LearnTypeDef *pIR_Packet)
{
    uint16_t  BaseTime = 1000 / pIR_Packet->freq;
    for(uint16_t i = 0;i < pIR_Packet->buf_index;i++){
        if((i%2) == 0){
            pIR_Packet->ir_buf[i] = PULSE_HIGH | HL_Time2TxBufCount(pIR_Packet->ir_buf[i], BaseTime);
        }
        else{
            pIR_Packet->ir_buf[i] = PULSE_LOW | HL_Time2TxBufCount(pIR_Packet->ir_buf[i], BaseTime);
        }
        // APP_PRINT_INFO2("[SmartRcu_ir_Decode] pIR_Packet->ir_buf[%d] = %x",i,pIR_Packet->ir_buf[i]);
    }
