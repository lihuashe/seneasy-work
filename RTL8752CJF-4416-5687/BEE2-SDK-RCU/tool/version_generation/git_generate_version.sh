...
# $1 is the prefix of Git Tag, and default is "bee3-sdk-v"
if [ ! $1 ]; then
    TAG_HEADER="bee3-sdk-v"
else
    TAG_HEADER=$1v
fi
TAG_HEADER_LEN=`echo $TAG_HEADER | wc -c`
#echo $TAG_HEADER       #bee3-sdk-v
...
# modify TAG_HEADER due to multiple version tag exist in bee3
...
new_version=`cat $TEMPLATE_PATH | sed -e "s/MAJOR_T/$MAJOR/g" \
                     -e "s/MINOR_T/$MINOR/g" \
...
# 不比较building time
old_version=`cat $VERSION_PATH | sed -e '15d'`
...
`cat $TEMPLATE_PATH | sed -e "s/MAJOR_T/$MAJOR/g" \
                     ...
                     -e "s/CN_8_T/$CN_8/g" \
                     > $VERSION_PATH`
echo "Generated Version File... version=$CUSTOMER_NAME"_"$MAJOR"."$MINOR"."$REVISION"."$BUILDNUM:"
fi
