...
typedef union
{
    ...
    struct
    {
        ...
        uint8_t rsvd : 4;                        /* bit[1:4]: reserved for ROM code */
        uint8_t test_mode : 3;                   /* bit[5:7]: test mode */
    } s;
...
static inline bool dfu_check_nw_dfu_mode_flag(void)
{
    T_BTAON_FAST_TEST_MODE_TYPE nFastBoot =
        (T_BTAON_FAST_TEST_MODE_TYPE)btaon_fast_read_safe(BTAON_FAST_TEST_MODE);
...
static inline void dfu_set_nw_dfu_mode_flag(bool enable)
{
    ...
    btaon_fast_write_safe(BTAON_FAST_TEST_MODE, nFastBoot.d8);
