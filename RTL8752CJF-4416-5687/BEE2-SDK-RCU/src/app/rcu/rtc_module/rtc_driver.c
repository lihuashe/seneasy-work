...
#define RTC_OVERFLOW_TIME_IN_MS (((uint32_t)1 << 24) * (1 + RTC_PRESCALER_VALUE) / 32)
...
uint64_t rtc_driver_get_us(void)
{
    ...
    time_in_us = ((1 << 24) * rtc_ovf_cnt + RTC_GetCounter()) * 1000 * (1 + RTC_PRESCALER_VALUE) / 32;
...
bool rtc_driver_set_comp_0_params(uint32_t time_ms, rtc_driver_raw_cb cb_func)
{
    ...
    if ((time_ms == 0) || (time_ms >= RTC_OVERFLOW_TIME_IN_MS))
    {
        result =  false;
    }
    else
    {
        ...
        comp_value = time_ms * 32 / (1 + RTC_PRESCALER_VALUE);
        comp_value = ((RTC_GetCounter() + comp_value) & 0xFFFFFF);
        RTC_SetComp(0, comp_value);
        RTC_ClearCompINT(0);
...
void RTC_Handler(void)
{
    ...
    if (LPC_GetINTStatus(LPC_INT_COUNT_COMP) == SET)
    {
        ...
        DBG_DIRECT("[RTC_Handler] LPC lower power triggered!");

        WDG_SystemReset(RESET_ALL, RESET_REASON_LPC_TRIGGER);
    }
}
