...
#define RTC_APP_CALLBACK_ENABLE   1

#define APP_CB_NUMBERS              7
...
#if (RTC_APP_CALLBACK_ENABLE == 1)
APP_MAIN_FUNC user_rtc_handler_cb __attribute__((weak)) = NULL;
void app_cb6_rtc_handler(void);
#endif
...
#if (RTC_APP_CALLBACK_ENABLE == 1)
    .app_cb_addr[6] = (uint32_t)app_cb6_rtc_handler,
#else
    .app_cb_addr[6] = (uint32_t)NULL,
#endif
    //add more cb here
};
#if defined ( __CC_ARM   )
...
#if DISABLE_FTL_REATURE
bool patch_ftl_init_imp(uint32_t u32PageStartAddr, uint8_t pagenum) APP_FLASH_TEXT_SECTION;
bool patch_ftl_init_imp(uint32_t u32PageStartAddr, uint8_t pagenum)
{
    extern uint8_t g_PAGE_num;
    g_PAGE_num = 0;

    /*do nothing, not init ftl*/
    return true;
}
#endif
...
void SystemInit(void)
{
    ...
    patch_ftl_init = patch_ftl_init_imp;
#endif
    ...
    OTP->bkp_data2_addr = USER_DATA2_START_ADDR;
    OTP->bkp_data2_size = USER_DATA2_MAX_SIZE;
#endif
...
void print_reset_reason(void)
{
    ...
    {
        BOOT_PRINT_INFO0("[Debug Info]RESET Reason: HW or OTA");
    }
    else
    {
        ...
        {
            BOOT_PRINT_INFO1("[Debug Info]RESET Reason: SW(reset except aon), TYPE 0x%x", sw_reset_type);
        }
        else //reset all will clear aon register
        {
            BOOT_PRINT_INFO1("[Debug Info]RESET Reason: SW(reset all), TYPE 0x%x",
...
void app_cb6_rtc_handler()
{
    if (user_rtc_handler_cb)
    {
        if (user_rtc_handler_cb())
        {
            return;
