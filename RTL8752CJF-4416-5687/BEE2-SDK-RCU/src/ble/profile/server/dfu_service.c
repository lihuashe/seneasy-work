...
#pragma diag_warning=Pa039
...
#if (SUPPORT_TEMP_COMBINED_OTA == 1)
            result = sil_dfu_update(g_dfu_para.ctrl_header.image_id,
...
#else
            result = sil_dfu_update(g_dfu_para.ctrl_header.image_id, g_dfu_para.cur_offset,
            ...
            else
            {
                ...
                result = sil_dfu_flash_erase_with_retry(g_dfu_para.ctrl_header.image_id,
                                                        g_sil_dfu_resend_offset, FMC_SEC_SECTION_LEN);
                if (result)
                {
                    //erase fail
...
{
    if (g_dfu_para.dfu_conn_para_upd_in_progress == true)
    {
...
void dfu_service_handle_valid_fw(uint8_t conn_id)
{
    ...
    {
        check_result = silent_dfu_check_checksum(g_dfu_para.ctrl_header.image_id, 0);
    }
    else
    {
        check_result = silent_dfu_check_checksum(temp_image_info[temp_image_num].image_id,
                                                 temp_image_info[temp_image_num].image_offset);
    }
#else
    check_result = silent_dfu_check_checksum(g_dfu_para.ctrl_header.image_id);
#endif
...
void dfu_service_handle_active_image(void)
{
    ...
    T_IMG_ID image_id = (T_IMG_ID)g_dfu_para.ctrl_header.image_id;
    ...
    if (IMAGE_USER_DATA == image_id || IMAGE_USER_DATA2 == image_id)
    {
        ...
        {
            p_header = (T_IMG_CTRL_HEADER_FORMAT *)(flash_get_bank_addr(FLASH_BKP_DATA2) |
                                                    FLASH_OFFSET_TO_NO_CACHE);
        }

        dfu_set_image_ready(p_header);
    }
    else // not user data image
    {
        ...
        else
        {
            //support bank switch
#if (SUPPORT_OTA_APP_DATA_EXTENSION == 1)
            for (uint16_t img_id = IMAGE_MAX - 1; img_id >= OTA; img_id--)
#else
            for (uint16_t img_id = AppData2; img_id >= OTA; img_id--)
#endif
            {
                if (GET_VALID_BITMAP(img_id))
                {
#if (SUPPORT_OTA_APP_DATA_EXTENSION == 1)
                    p_header = (T_IMG_CTRL_HEADER_FORMAT *)flash_ioctl(flash_ioctl_get_temp_bank_addr_by_image_id_ext,
                                                                       (T_IMG_ID)img_id, 0);
#else
                    /* check OTA temp to see if received image is OK.*/
                    p_header = (T_IMG_CTRL_HEADER_FORMAT *)get_temp_ota_bank_addr_by_img_id((T_IMG_ID)img_id);
#endif
                    dfu_set_image_ready(p_header);
                }
            }
...
#pragma diag_warning=Pa039
            ...
            DFU_PRINT_TRACE6("DFU_OPCODE_START_DFU: ic_type=0x%x, secure_version=0x%x, ctrl_flag.value=0x%x, image_id=0x%x,crc16=0x%x, payload_len=%d",
...
#ifdef SDK_8772
            if (g_dfu_para.ctrl_header.ic_type == 0x09)
#else
            if (g_dfu_para.ctrl_header.ic_type == 0x05)
#endif
            {
                if (((g_dfu_para.ctrl_header.image_id >= OTA) && (g_dfu_para.ctrl_header.image_id < IMAGE_MAX))
                    || (IMAGE_USER_DATA == g_dfu_para.ctrl_header.image_id)
                    || (IMAGE_USER_DATA2 == g_dfu_para.ctrl_header.image_id))
                {
#if (SUPPORT_TEMP_COMBINED_OTA == 1)
...
//                        temp_image_info[temp_image_num].image_size = UP_ALIGN(g_dfu_para.image_total_length, FMC_SEC_SECTION_LEN);
                    ...
                    unlock_flash_all();
#if (SUPPORT_TEMP_COMBINED_OTA == 1)
                    uint32_t result = sil_dfu_update(g_dfu_para.ctrl_header.image_id,
                                                     0 + temp_image_info[temp_image_num].image_offset, DFU_HEADER_SIZE,
                                                     (uint32_t *)&g_dfu_para.ctrl_header, true);
#else
                    uint32_t result = sil_dfu_update(g_dfu_para.ctrl_header.image_id, 0, DFU_HEADER_SIZE,
                                                     (uint32_t *)&g_dfu_para.ctrl_header.ic_type);
#endif
                    lock_flash();
                    if (result == 0)
                    {
                    ...
                    {
                        DBG_DIRECT("DFU_OPCODE_START_DFU: sil_dfu_update error=%d", result);
                        silent_dfu_reset(g_dfu_para.ctrl_header.image_id);
                        dfu_fw_active_reset();
                    }
                ...
                {
                    DFU_PRINT_ERROR1("DFU_OPCODE_START_DFU: signature=0x%x invalid", g_dfu_para.ctrl_header.image_id);
                    notif_data[0] = DFU_OPCODE_NOTIFICATION;
                    notif_data[1] = DFU_OPCODE_START_DFU;
                    notif_data[2] = DFU_ARV_FAIL_INVALID_PARAMETER;
                    server_send_data(conn_id, dfu_service_id, INDEX_DFU_CONTROL_POINT_CHAR_VALUE, \
            ...
            {
                DFU_PRINT_ERROR1("DFU_OPCODE_START_DFU: ic_type=0x%x invalid", g_dfu_para.ctrl_header.ic_type);
                notif_data[0] = DFU_OPCODE_NOTIFICATION;
                notif_data[1] = DFU_OPCODE_START_DFU;
                notif_data[2] = DFU_ARV_FAIL_INVALID_PARAMETER;
                server_send_data(conn_id, dfu_service_id, INDEX_DFU_CONTROL_POINT_CHAR_VALUE, \
    ...
    case DFU_OPCODE_VALID_FW://0x03
        ...
        {
            LE_ARRAY_TO_UINT16(g_dfu_para.ctrl_header.image_id, p);
            DFU_PRINT_TRACE1("DFU_OPCODE_VALID_FW: signature=0x%x", g_dfu_para.ctrl_header.image_id);
    ...
    case DFU_OPCODE_REPORT_TARGET_INFO://0x06
        ...
        {
            LE_ARRAY_TO_UINT16(g_dfu_para.ctrl_header.image_id, p);
            PROFILE_PRINT_INFO1("DFU_OPCODE_REPORT_TARGET_INFO: signature=0x%x",
            ...
            else if (IMAGE_USER_DATA2 == g_dfu_para.ctrl_header.image_id)
            {
                flash_read_locked(flash_get_bank_addr(FLASH_BKP_DATA2) + CAL_OFFSET(T_IMG_HEADER_FORMAT, git_ver),
                                  4,
                                  (uint8_t *)&g_dfu_para.origin_image_version);
            }
            else
    ...
    case DFU_OPCODE_CONN_PARA_TO_UPDATE_REQ://0x07
        {
            ...
            {
                if (g_dfu_para.dfu_conn_para_upd_in_progress)
                {
                ...
                else
                {
                                             ...
                                             superv_tout, conn_interval_min * 2 - 2, conn_interval_max * 2 - 2) == GAP_CAUSE_SUCCESS)
                    {
                        ...
                        g_dfu_para.dfu_conn_para_upd_in_progress = true;
    ...
    case DFU_OPCODE_COPY_IMG://0x0c
        {
            ...
            if (sil_dfu_copy_img(g_dfu_para.ctrl_header.image_id, dlAddress, dlSize))
            {
                notif_data[2] = DFU_ARV_SUCCESS;
            }
            else
    ...
    {
        dfu_service_handle_control_point_req_cb(conn_id, dfu_control_point.opcode);
    }
...
#pragma diag_warning=Pa039
            ...
            g_dfu_para.cur_offset + g_ota_tmp_buf_used_size == g_dfu_para.image_total_length)
        {
...
#else
            uint32_t result = sil_dfu_update(g_dfu_para.ctrl_header.image_id, g_dfu_para.cur_offset,
                                             g_ota_tmp_buf_used_size,
                                             (uint32_t *)p_ota_temp_buffer_head);
#endif
            lock_flash();
            ...
            else
            {
                ...
                DBG_DIRECT("dfu_service_handle_packet_req: sil_dfu_update error=%d", result);
                silent_dfu_reset(g_dfu_para.ctrl_header.image_id);
                //WDG reset system
                dfu_fw_active_reset();
            }
...
uint8_t dfu_add_service(void *pFunc)
{
    if (false == server_add_service(&dfu_service_id,
                                    ...
                                    DfuServiceCBs))
    {
        PROFILE_PRINT_ERROR1("dfu_add_service fail: dfu_service_id=%d", dfu_service_id);
        dfu_service_id = 0xff;
