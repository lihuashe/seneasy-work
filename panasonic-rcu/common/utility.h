...
typedef struct {
    u8 *buffer;     // 缓冲区指针
    u32 size;       // 缓冲区总大小
    volatile u32 head; // 写入位置（需volatile保证多线程可见性）
...
u32 RingBuffer_Avail(const RingBuffer *rbuf);  // 可用空间
u32 RingBuffer_Used(const RingBuffer *rbuf);   // 已用空间
...
// 高级功能
u32 RingBuffer_Peek(const RingBuffer *rbuf, u8 *data, u32 max_len); // 查看数据不移除
void     RingBuffer_Clear(RingBuffer *rbuf); // 清空缓冲区
