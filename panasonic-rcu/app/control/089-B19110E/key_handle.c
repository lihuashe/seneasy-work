...
const static KeyEvent key_release_cb_arr[] = {
    ...
    KEY_RELEASE_EVENT_ITEM(7, time_key_release_cb),
};

static void time_add_key_cb(u32 time_ms, u8 key);
static void time_sub_key_cb(u32 time_ms, u8 key);
const static KeyEvent key_repeat_cb_arr[] = {
    KEY_REPEAT_EVENT_ITEM(4, time_add_key_cb),
    KEY_REPEAT_EVENT_ITEM(7, time_sub_key_cb),
};
...
static void time_sub_key_cb(u32 time_ms, u8 key)
{
    ...
    {
        last_time_ms = 0;
        timing_add_or_sub(false, false);
    }
    else if (time_ms >= 1000 && time_ms - last_time_ms > TIME_SET_REPEAT_INTERVAL_MS)
    {
        timing_add_or_sub(false, true);
        last_time_ms = time_ms;
    }
}
