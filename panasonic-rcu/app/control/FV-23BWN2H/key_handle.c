...
static void nanoeX_mode_key_cb(u8 key);
...
const static KeyEvent click_key_cb_arr[] = {
    ...
    KEY_CLICK_EVENT_ITEM(NANOE_X_K, nanoeX_mode_key_cb),
    KEY_LONG_CLICK_EVENT_ITEM(POWER_K, 3000, reset_key_cb),
    KEY_COMB_LONG_CLICK_EVENT_ITEM(HEAT_K, HEAT_K, 3000, pair_key_cb),
...
const static KeyEvent key_release_cb_arr[] = {
    ...
    KEY_RELEASE_EVENT_ITEM(TIME_ADD_K, time_key_release_cb),
    KEY_RELEASE_EVENT_ITEM(TIME_SUB_K, time_key_release_cb),
};
...
const static KeyEvent key_repeat_cb_arr[] = {
    KEY_REPEAT_EVENT_ITEM(TIME_ADD_K, time_add_key_cb),
    KEY_REPEAT_EVENT_ITEM(TIME_SUB_K, time_sub_key_cb),
};
...
static void nanoeX_mode_key_cb(u8 key)
{
    ...
    my_log(">D%d[%s] key:%d\n", __LINE__, __FUNCTION__, key);
#endif
    change_mode(MODE_NANOE_X);
    send_ctrl_data(RF_SEND_NANOE_X_FUN);
}
...
static void wind_direction_key_cb(u8 key)
{
    ...
    if (g_current_ctrl.main_mode == MODE_STOP ||
        ...
        g_current_ctrl.main_mode == MODE_NANOE_X)
        return;
    air_dir_change();
