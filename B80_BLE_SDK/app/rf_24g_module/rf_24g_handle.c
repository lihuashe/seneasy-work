...
void check_rf_complet_status(void)
{
	...
	else
	{
		...
		if (ack_miss_no >= 2)
		{
			if(app_factory_channel_flag == 0)
				device_channel = get_next_channel_with_mask(0, device_channel);
			else
			{
				if(app_factory_channel_flag == 0x01)
					device_channel = 2;					//2413
				else if(app_factory_channel_flag == 0x02)
					device_channel = 5;					//2426
				else if(app_factory_channel_flag == 0x03)
					device_channel = 7;					//2435
				else if(app_factory_channel_flag == 0x04)
					device_channel = 11;				//2455
				else if(app_factory_channel_flag == 0x05)
					device_channel = 15;				//2476
			}
		}
