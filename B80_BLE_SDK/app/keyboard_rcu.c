...
MYFIFO_INIT (fifo_km, 60, 16);//The size must be a multiple of 4 bytes
...
_attribute_data_retention_ u32 app_ota_success_timer = 0;
_attribute_data_retention_ u8 app_factory_channel_flag = 0;
...
void rcu_main_loop(void)
{
...
#if (CHARGE_ENABLE)
    if(app_rcu_charge_check_timer && clock_time_exceed(app_rcu_charge_check_timer, 500*1000)){//500ms
        app_rcu_charge_check_timer = 0;
        app_check_charge_batt();
    }
    if(app_rcu_charge_out_timer && clock_time_exceed(app_rcu_charge_out_timer, 100*1000)){//100ms
		app_rcu_charge_out_flag ++;
        if(app_rcu_charge_out_flag <= 25)
		{
			app_rcu_charge_out_timer = clock_time() | 1;
			state_of_charge();
		}
		else
		{
			app_rcu_charge_out_timer = 0;
			app_rcu_charge_out_flag = 0;
		}
    }
#endif

    if(app_rcu_main_init_timer && clock_time_exceed(app_rcu_main_init_timer, 500*1000)){//500ms
		#if (TOUCH_ENABLE)
			FT3308_iic_init();
		#endif
    ...
    if(app_rcu_kb_pressed_timer && clock_time_exceed(app_rcu_kb_pressed_timer, 500*1000)){//500ms
    ...
    if(app_factory_mode_timer && clock_time_exceed(app_factory_mode_timer, 100*1000))//8min
    {
		factory_report_cnt ++;
		if(factory_report_cnt >= 4800)
		{
			app_factory_channel_flag = 0;
			device_channel = 0;
	...
	if(app_ota_success_timer && clock_time_exceed(app_ota_success_timer, 3000*1000))
    { 
		app_ota_success_timer = 0; 
		gpio_write(GPIO_LED_1,!LED_ON_LEVAL);
		start_reboot();
    }
}
		...
		else if(device_status == STATE_NORMAL)
		{
			...
			{ 
				...
				if(p)
				{
					...
					#if (FACTORY_ENABLE)
					if(factory_mode_flag)
						memcpy(&km_dat1->km_dat[0], &tmp[2], 6+40);
					else
			...
			else if(key_num == 2)
			{
				...
				if((key0 == KEY_FN) || (key1 == KEY_FN))
				{
					if(key0 == KEY_FN)
					{
						...
						case KEY_EQUAL:	case KEY_Q:		case KEY_T:		case KEY_Y:		case KEY_U: \
						case KEY_I:		case KEY_W:		case KEY_J:		case KEY_L:		case KEY_SLASH:
							temp_buff[4] = HID_KB_SHIFT_LEFT;
							temp_buff[5] = KB_Map_RCU_HID_FN[key1];
							break;
						case KEY_P:	case KEY_E:	case KEY_R:	case KEY_O:	case KEY_H:	case KEY_K:	case KEY_PERIOD: \
						case KEY_ARROW_UP:	case KEY_ARROW_DOWN:	case KEY_ARROW_LEFT:	case KEY_ARROW_RIGHT:	case KEY_DELETE:
							temp_buff[5] = KB_Map_RCU_HID_FN[key1];
							break;	
						case KEY_ENTER:	
					...
					if(key1 == KEY_FN)
					{
						...
						case KEY_6:		case KEY_7:		case KEY_8:		case KEY_9:		case KEY_0: \
						case KEY_EQUAL:	case KEY_Q:		case KEY_T:		case KEY_Y:		case KEY_U: \
						case KEY_I:		case KEY_P:		case KEY_J:		case KEY_L:		case KEY_SLASH:
							temp_buff[4] = HID_KB_SHIFT_LEFT;
							temp_buff[5] = KB_Map_RCU_HID_FN[key0];
							break;
						case KEY_W:	case KEY_E:	case KEY_R:	case KEY_O:	case KEY_H:	case KEY_K:	case KEY_PERIOD: \
						case KEY_ARROW_UP:	case KEY_ARROW_DOWN:	case KEY_ARROW_LEFT:	case KEY_ARROW_RIGHT:	case KEY_DELETE:
							temp_buff[5] = KB_Map_RCU_HID_FN[key0];
							break;	
						case KEY_ENTER:
...
void get_data_report_device_fwversion_Gsensor(u8 test1, u8 test2, u8 test3, u8 test4, u8 test5, u8 test6)
{
	if(rcu_ota_dfu_flag >= 0x02)
		return;
	my_fifo_reset(&fifo_km);
...
void get_data_report_device_mac(void)
{
	...
	if(factory_mode_flag)
	{
		...
		#if (MODULE_WATCHDOG_ENABLE)
		wd_set_interval_ms(30*1000,CLOCK_SYS_CLOCK_1MS);
		wd_start();
		#endif
	}
}
	   ...
	   (stuck_ear_sleep_ck_time && clock_time_exceed(stuck_ear_sleep_ck_time ,  rf_24G_deep_time_ms*1000)))
	{
		...
		if((need_suspend_flag == 0x01) && (key_not_released == 0) && (pair_flag == 0) && (rcu_ota_dfu_flag == 0) && (factory_mode_flag == 0))
		{
			...
			if(rcu_wakeup_touch)
			{
				...
				#if(GSENSOR_DA217_ENABLE)
				cpu_set_gpio_wakeup(DA217_INT1, int_wake_up_level, 0);
				#endif
