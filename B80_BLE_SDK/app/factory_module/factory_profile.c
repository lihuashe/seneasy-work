...
_attribute_data_retention_ u8 factory_buf_aaa[20+40];
void report_factory_status(u8 factory_status, u8 *raw_buf)
{
    ...
    if(factory_status == 0x00)
        ...
        memcpy(factory_buf_aaa, raw_buf, 10+40); 
	...
	if ((device_status == STATE_NORMAL)) {
		reset_idle_status();
		my_fifo_push(&fifo_km, factory_buf_aaa, 6+40);//6
	}		
}
...
void recv_factory_data(u8 *factory_buf)
{
    ...
    if((factory_buf[0] == 0x55) && (factory_buf[1] == 0xAA))
    {
        ...
        case CMD_REQ_SEND_TX_DATA:
            ...
            break;
        case CMD_REQ_TOUCH_RAW:
            touch_read_raw_data(touch_buff);
            raw_buff[4] = factory_buf[6];
            if(factory_buf[6] == 0xAA)//CH:1-17
            {
                for (int i = 0; i < 34; i++)
                    raw_buff[5 + i] = touch_buff[i + 2];
            }
            else if(factory_buf[6] == 0xBB)//CH:18-35
            {
                for (int i = 0; i < 36; i++)
                    raw_buff[5 + i] = touch_buff[i + 36];
            }
            break;
        case CMD_REQ_TOUCH_CB:
            touch_read_cb_data(touch_buff);
            raw_buff[4] = factory_buf[6];
            if(factory_buf[6] == 0xAA)//CH:1-17
            {
                for (int i = 0; i < 34; i++)
            ...
            break;
        case CMD_REQ_TOUCH_SHORT:
            touch_read_short_data(touch_buff);
            raw_buff[4] = factory_buf[6];
            if(factory_buf[6] == 0xAA)//CH:1-17
            {
                for (int i = 0; i < 34; i++)
            ...
            break;
        case CMD_REQ_TOUCH_NOISE:
            touch_read_noize_data(touch_buff);
            raw_buff[4] = factory_buf[6];
            if(factory_buf[6] == 0xAA)//CH:1-17
            {
                for (int i = 0; i < 34; i++)
            ...
            break;
        case CMD_REQ_TOUCH_DIFF:
            touch_read_diff_data(touch_buff);
            raw_buff[4] = factory_buf[6];
            if(factory_buf[6] == 0xAA)//CH:1-17
            {
                for (int i = 0; i < 34; i++)
