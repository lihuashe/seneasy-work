...
_attribute_data_retention_ u32 app_rcu_charge_out_timer = 0;
_attribute_data_retention_ u8 app_rcu_charge_out_flag = 0;
...
charge_stop_t charge_stop = CHARGE_STOP_INIT;

void voltage_stop_charge_handle(void)
{
    volatile u8 buff = 0;
    i2c_gpio_set(GPIO_PD1,GPIO_PD0);
    i2c_master_init(LP40801B_DEVICE_ADDR, (unsigned char)(CLOCK_SYS_CLOCK_HZ/(4*100000)));
    buff = ~((~i2c_read_byte(LP4081_REG_CTRL2, 1)) | 0x80);
    i2c_write_byte(LP4081_REG_CTRL2, 1, buff);
    printf("--->>> voltage_stop_charge_handle : %1x\r\n",buff);
}
...
void voltage_start_charge_handle(void)
{
    ...
    buff = ((i2c_read_byte(LP4081_REG_CTRL2, 1)) | 0x80);
    i2c_write_byte(LP4081_REG_CTRL2, 1, buff);
    printf("--->>> voltage_start_charge_handle : %1x\r\n",buff);
}

void adc_time_multplex_read(void)
{
    if (!app_rcu_charge_flag)
        memset(&charge_stop, 0 ,sizeof(charge_stop));
    
    //when MCU powered up or wakeup from deep/deep with retention, adc need be initialized
    if (!charge_stop.tick_flag)
    {
        charge_stop.tick_flag = true;
        charge_stop.tick = clock_time()|1;
        if(!adc_hw_initialized)
        {
            adc_hw_initialized = 1;
            adc_vbat_detect_init(1);
        }
    }
    if (charge_stop.tick && clock_time_exceed(charge_stop.tick, 5 * 100 * 1000))
    {
        charge_stop.tick = clock_time();
        if (!adc_hw_initialized) 
        {
            adc_hw_initialized = 1;
            adc_vbat_detect_init(1);
...
void adc_vbat_detect_init(bool chose)
{
    ...
    {
        gpio_set_output_en(GPIO_NIC_DETECT, 1);
        gpio_set_input_en(GPIO_NIC_DETECT,1);
        gpio_set_output_en(GPIO_NIC_DETECT, 0);
        adc_set_ain_chn_misc(ADC_INPUT_PCHN_PB6, GND);
    }
...
///// get average value from raw data(abandon some small and big data ), then filter with history data //////
    ...
    if (!app_rcu_charge_flag || adc_hw_initialized)
    {
        batt_vol_mv = (adc_result * g_adc_vref)>>10;
        batt_vol_mv = ((u32)(batt_vol_mv*(1000+180+1000)))/1000;  //检测到的电压 再按分压得到实际电压
        printf("--->>> batt_vol_mv : %d\r\n",batt_vol_mv);
    }
    else 
    {
        charge_stop.batt_nic_mv = (adc_result * g_adc_vref)>>10;
        charge_stop.batt_nic_mv = ((u32)(charge_stop.batt_nic_mv*(1000+100+1000)))/2090;  //检测到的电压 再按分压得到实际电压
        printf("--->>> charge_stop.batt_nic_mv : %d\r\n",charge_stop.batt_nic_mv);
        if (((charge_stop.batt_nic_mv > 100) && (charge_stop.batt_nic_mv <= CHARGE_STOP_MIN_TMP)) || charge_stop.batt_nic_mv >= CHARGE_STOP_MAX_TMP)
        {
            charge_stop.dat_filter++;
            if (charge_stop.dat_filter >= 4)
            {
                voltage_stop_charge_handle();
            } 
        }
    }
