		...
		if(ota_cmd == 0x02)		
		{
			...
			if(ret==0)
			{
				...
				if (found) {
					...
					app_ota_success_timer = clock_time();
				} else {
		...
		else 
		{
            ...
            //printf("\r\n");
            {
                ...
                if(ota_error_flag==OTA_SUCCESS)
                {
					...
					u8 read_flash_buf_AAA[40];  
					flash_read_page(flash_write_addr + ota_program_offset, 16+16, read_flash_buf_AAA);
					if(memcmp(read_flash_buf_AAA, ota_buf, 32))
					{
						printf(" ######### do not equal  !!!!!!!!!!!!!\r\n");
						printf("READ_CHECK_BUFF: \r\n");
						for(u16 i = 0; i < 32; i++)
							printf("%01x ", read_flash_buf_AAA[i]);
						printf("\r\n");
			
						flash_write_page(flash_write_addr + ota_program_offset, 16+16, ota_buf);
						flash_read_page(flash_write_addr + ota_program_offset, 16+16, read_flash_buf_AAA);
						printf("READ_CHECK_BUFF_222: \r\n");
						for(u16 i = 0; i < 32; i++)
							printf("%01x ", read_flash_buf_AAA[i]);
						printf("\r\n");						
					}
