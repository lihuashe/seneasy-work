...
u8 chn_idx = 0;
u8 test_mode_sel = 0;
extern u8 factory_test_mode_flag;
typedef enum 
{
	CARRIER_MODE=0,
	CARRIER_DATA_MODE=1,
...
extern volatile u32 tick_factory_tx_data;
void usb_host_cmd_proc(void)
{
	if(factory_test_mode_flag == FACTORY_MODE_STATE && (tick_factory_tx_data && clock_time_exceed(tick_factory_tx_data, 500000))){
		tick_factory_tx_data = 0;
		printf("factory_test_mode_flag == FACTORY_MODE_STATE\n\r");	
		// irq_disable();
		...
		app_rf_emi_single_tone(pwr,42);
	}
	if(emi_flg)
	{
		while(1)
		{
			factory_version_ir_send();
