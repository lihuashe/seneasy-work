...
{
	g_adc_vref = g_adc_gpio_calib_vref; 
	g_adc_vref_offset = g_adc_gpio_calib_vref_offset;
...
unsigned int adc_sample_and_get_result(void)
{
	...
	while(!clock_time_exceed(t0, 120));  //wait at least 2 sample cycle(f = 96K, T = 10.4us)
//////////////// get adc sample data and sort these data ////////////////
	for(i=0;i<ADC_SAMPLE_NUM;i++){
	 ...
	 The default data is already ready.*/
		while((!adc_data_buf[i])&&(!clock_time_exceed(t0,120)));
		 t0 = clock_time();
